...
if (!process.env.API_KEY) {
    state.error = "API_KEY environment variable is not set. Please configure it to use QuizBot.";
}
...
const handleGenerateQuiz = async () => {
    if (!state.topic.trim() || !process.env.API_KEY) return;
...
const renderQuizForm = () => `
  <div class="form-container">
    <form id="quiz-form">
      ...
      <button type="submit" id="generate-quiz-btn" class="btn btn-primary" ${state.loading || !state.topic.trim() ? 'disabled' : ''}>
...
const renderScoreSummary = () => {
    if (!state.quiz) return ''; // Should not happen due to render logic, but good for type safety
...
const render = () => {
    ...
    if (state.loading) {
        ...
        mainContent = `<div style="max-width: 28rem; margin: 0 auto;">${renderQuizForm()}</div>`;
        if (state.error) {
           mainContent += renderError(state.error);
        }
    }
    
    // In initial state, show form also if API key is missing.
    if (!state.quiz && !state.loading && state.error && !process.env.API_KEY) {
        mainContent = `
...
root.addEventListener('input', e => {
    ...
    if (target.id === 'topic') {
        ...
        if (generateBtn) {
            generateBtn.disabled = state.loading || !state.topic.trim();
        }
    }
